...
const newComponent = `function Testimonials({ onContact }: { onContact: () => void }) {
  ...
  // Setar estado local caso queiramos adicionar indicativos ativos
  ...
  return (
    ...
    >
      <div className="grid gap-10 lg:grid-cols-[380px_minmax(0,1fr)] xl:grid-cols-[420px_minmax(0,1fr)] lg:gap-14 items-start">
        ...
        <motion.div 
          className="relative min-w-0 w-full lg:-mr-[100vw]"
          initial={reduced ? undefined : { opacity: 0, x: 30 }}
          whileInView={reduced ? undefined : { opacity: 1, x: 0 }}
          viewport={{ once: true }}
          transition={{ duration: 0.8, ease: "easeOut", delay: 0.2 }}
        >
          <div className="overflow-hidden lg:pr-[100vw]" ref={emblaRef}>
            <div className="flex touch-pan-y gap-6 sm:gap-8 py-4 px-2" style={{ backfaceVisibility: 'hidden' }}>
              {items.map((t, index) => (
                ...
                >
                  <div className={\`flex flex-col h-full rounded-[32px] p-6 sm:p-8 transition-all duration-500 \${selectedIndex === index ? 'bg-white shadow-xl shadow-black/5 ring-1 ring-zinc-200 scale-100' : 'bg-zinc-50 ring-1 ring-zinc-100 scale-[0.98] opacity-60 hover:opacity-100'}\`}>
