  ...
  return (
    ...
    >
      <div className="grid gap-10 lg:grid-cols-[380px_minmax(0,1fr)] xl:grid-cols-[420px_minmax(0,1fr)] lg:gap-14 items-center">
        ...
        >
          {/* overflow-hidden é OBRIGATÓRIO para o Embla não sobrepor os outros elementos! */}
          <div className="overflow-hidden rounded-3xl" ref={emblaRef}>
            <div className="flex touch-pan-y gap-6 py-4 px-2" style={{ backfaceVisibility: 'hidden' }}>
              {items.map((t, index) => (
                <div
                  key={\`testimonial-slide-\${t.id}\`}
                  className="relative shrink-0 w-[85%] sm:w-[320px] lg:w-[340px] h-full cursor-grab active:cursor-grabbing"
                  onClick={() => emblaApi?.scrollTo(index)}
                >
                  <div className={\`flex flex-col h-full rounded-[32px] p-6 sm:p-8 transition-all duration-500 \${selectedIndex === index ? 'bg-white shadow-xl shadow-black/5 ring-1 ring-zinc-200 scale-100 z-10' : 'bg-zinc-50 ring-1 ring-zinc-100 scale-[0.96] opacity-60 hover:opacity-100 z-0'}\`}>
          ...
          <div className="mt-6 flex items-center gap-2 pl-2">
            <button
              ...
              className="pointer-events-auto grid h-10 w-10 place-items-center rounded-full bg-white shadow-sm ring-1 ring-zinc-200 text-zinc-600 transition hover:bg-zinc-50 hover:text-zinc-900 active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none"
              aria-label="Anterior"
            >
...
if (!regex.test(content)) {
  console.log("Could not find Testimonials component to replace!");
  process.exit(1);
}

const newContent = content.replace(regex, newComponent);
fs.writeFileSync(filePath, newContent);
console.log("Fixed Testimonials layout and carousel.");
