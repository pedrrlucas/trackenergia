...
// O objetivo é modernizar o footer.
// Remover 'Ver todas' em soluções.
// Mudar a estética: cores mais fechadas (tudo dark, minimalista), fontes mais refinadas,
// ícones sutis. O design atual tem um fundo `bg-gradient-to-r from-black via-[#12001f] to-[#1d0238]`.
// Faremos um background escuro contínuo `#0a0014` com linhas guias subtis.

const newFooter = `<footer id="footer" className="w-full bg-[#0a0014] relative border-t border-white/5 overflow-hidden">
      {/* Decorações do Footer para conversar com o site */}
...
const footerRegex = /<footer id="footer".*?<\/footer>/s;

if (footerRegex.test(content)) {
  content = content.replace(footerRegex, newFooter);
  fs.writeFileSync(filePath, content);
  console.log("Footer successfully rewritten to modern dark style.");
} else {
  console.log("Could not match the footer block.");
}
